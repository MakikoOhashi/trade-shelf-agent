...
Shipment・Invoice・通関書類・Issue状態を整理・把握しながら、
営業からの曖昧な問い合わせへ対応する
...
### Scenario 1: 状態変更 → approval → shelf state transition
...
### Scenario 2: 営業問い合わせ → clarification → PL missing → supplier followup approval
...
![Approval Center](docs/images/email.approval.png)
...
現状実装の詳細図は [docs/current-architecture.md](docs/current-architecture.md) を参照してください。
...
Processor / agent 設計:
[docs/agent-processors.md](docs/agent-processors.md)
