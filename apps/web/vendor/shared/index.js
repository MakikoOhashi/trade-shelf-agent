// Generated from packages/shared/src/{mockTradeCases.ts,incident.ts,canonical.ts,ingest.ts,mockInputs.ts}
...
function canonicalNormalizeSiId(siId, now = new Date()) {
...
function canonicalExtractEntityIdsFromText(text) {
    const t = String(text || "");
    const now = new Date();
...
function satisfiesMissingFields(rawText, missingFields) {
...
export function resolveCanonicalConversation(input, options = {}) {
    const pending = Array.isArray(options.pendingClarifications) ? options.pendingClarifications.filter(Boolean) : [];
    const direct = pending.find((p) => String(p?.sourceRawInputId || "") === String(input?.id || "") || String(p?.matchedRawInputId || "") === String(input?.id || ""));
    if (direct && direct.id) {
        return {
...
export function issueCandidateIdFromThreadId(threadId) {
...
export function resolveCanonicalIssueLink(thread, threadLinks = [], mode = "existing_or_candidate") {
    ...
    if (mode !== "candidate") {
        ...
        if (existing && existing.entityId)
            return { issueId: String(existing.entityId).trim(), reason: "linked_issue" };
    }
...
export function buildStateTransitionCandidates(args) {
    ...
    const hasUncertaintyLanguage = (text) => {
        const t = String(text || "").toLowerCase();
        return (/\b(maybe|might|expected|probably|likely|estimate|estimated)\b/.test(t) ||
            /(かも|可能性|見込み|たぶん|おそらく|予定)/.test(t));
    };
    const hasQuantityMismatchLanguage = (text) => {
        const t = String(text || "").toLowerCase();
        return /\b(quantity mismatch|qty mismatch|short|shortage)\b/.test(t) || /(数量差異|数量.*違|不足|ショート)/.test(t);
    };
    const hasDateMismatchLanguage = (text) => {
        const t = String(text || "").toLowerCase();
        return /\b(date mismatch|inconsistent date|conflicting date)\b/.test(t) || /(日付.*矛盾|日程.*矛盾|日付.*違)/.test(t);
    };
    const hasMissingDocumentLanguage = (text) => {
        const t = String(text || "").toLowerCase();
        return (/\b(missing document|document missing|bl missing|invoice missing|packing list missing)\b/.test(t) ||
            /(書類不足|書類未着|未着|未入手)/.test(t));
    };
    const hasExplicitShipmentIdInText = (text, shipmentId) => {
    ...
    const detectTransitionSignal = (text) => {
        const t = String(text || "").toLowerCase();
        ...
        if (/\b(arrived at warehouse|warehouse arrival|warehouse arrived|warehouse received|received at warehouse)\b/.test(t) ||
        ...
        if (/\b(customs in progress|in customs|under customs|under customs clearance|import customs)\b/.test(t) ||
    ...
    for (const thread of threads) {
        ...
        const scoreConfidence = (entityLink) => {
        ...
        const buildRisks = (entityLink, confidence, combinedText) => {
            const risks = [];
        ...
        const decide = (confidence, risks) => {
            ...
            return confidence >= 0.8 && risks.length === 0 ? "auto_apply" : "needs_issue_candidate";
        };
        const computeConfidenceAndRisks = (entityLink, combinedText) => {
            ...
            const isClearShippedUpdate = Boolean(shipmentId) &&
                explicitShipmentId &&
...
export function buildActivityEvents(input, threads, links, occurredAt, options = {}, stateTransitionCandidates = [], stateTransitionNotices = []) {
    ...
    for (const candidate of stCandidates) {
        ...
        const linked = (() => {
            ...
            if (entityType !== "Shipment" && entityType !== "SI" && entityType !== "Document")
                return undefined;
...
export function buildDraftDocuments(input, threads, links, intakeResolutions, issueMutations, actionPlans, options = {}) {
    ...
    for (const ap of plans) {
        ...
        const plReplyDraftBody = (() => {
            ...
            if (plSt === "missing")
                return `${inv} に紐づくPLは未着です。\n仕入先へ確認しますか？`;
            if (plSt === "received")
                return `${inv} に紐づくPLは登録済みです。`;
...
export function runIngestPipeline(input, options = {}) {
    ...
    const injectedLinks = (() => {
        ...
        if (!threadIds.length)
            return [];
        return threadIds.flatMap((threadId) => entities.map((e) => ({
