// Generated hotfix: keep in sync with packages/shared/src/relationshipResolver.ts
// NOTE: This file exists to ensure Azure ESM runtime can resolve the module at:
//   apps/web/vendor/shared/relationshipResolver.js

/**
 * @typedef {"missing" | "received" | "unknown"} PlStatus
 */
...
/**
 * @param {{ entityType: "Document" | "Shipment" | "SI"; entityId: string; tradeCases: any[] }} params
 * @returns {OperationalContextResolution | null}
 */
export function resolveOperationalContext(params) {
  ...
  const matched = (() => {
    ...
    if (entityType === "SI") {
      return (
        ...
        tradeCases.find(
          (tc) =>
            Array.isArray(tc?.siNumbers) &&
            tc.siNumbers.map((x) => String(x).toUpperCase()).includes(entityId),
        ) ||
        null
      );
    }
    ...
    if (invNo) {
      const byShipmentInvoices =
        tradeCases.find((tc) => {
          ...
          const invs = [...(sh?.supplierInvoices || []), ...(sh?.switchInvoices || [])]
            .map(normalizeInv)
            .filter(Boolean);
  ...
  const shipmentId =
    String(matched?.shipmentEntity?.id || "").trim() ||
    (Array.isArray(matched?.shipmentRefs) ? String(matched.shipmentRefs[0] || "") : "");
  const siNo =
    String(matched?.siEntity?.siNo || "").trim() ||
    (Array.isArray(matched?.siNumbers) ? String(matched.siNumbers[0] || "") : "");

  /** @type {PlStatus | undefined} */
  const plStatus = (() => {
    const docs = matched?.caseProgress?.documents;
    if (Array.isArray(docs)) {
      const pl = docs.find(
        (d) => String(d?.id || "").toLowerCase() === "pl" || String(d?.label || "").toUpperCase() === "PL",
      );
